...
int main() {
    ...
    vector<string> input_files = { "100x100.bmp", "200x200.bmp", "300x300.bmp", "400x400.bmp" };
    ...
    vector<int> ksize_values = { 7, 11, 15 };
...
void BMPRead(MyRGBTRIPLE**& rgb, MyBITMAPFILEHEADER& header, MyBITMAPINFOHEADER& bmiHeader, const char* fin) {
    ...
    fread(&header, sizeof(MyBITMAPFILEHEADER), 1, pFile);
    ...
    fread(&bmiHeader, sizeof(MyBITMAPINFOHEADER), 1, pFile);
    ...
    for (int i = 0; i < bmiHeader.biHeight; i++) {
        fread(&rgb[bmiHeader.biHeight - 1 - i][0], sizeof(MyRGBTRIPLE), bmiHeader.biWidth, pFile);
        if (padding > 0)
            fread(&tmp[0], sizeof(unsigned char), padding, pFile);
    }
